...
class UserlocationCompany extends Controller{
	var $language; 
	var $session;
	var $name;
	var $resturants;
	...
	Public function index(){
		...
		$page_url           = baseurl."admin/userlocationcompany/index/".$company_id; //URL
		...
		$this->db->flush_cache();
		$sql="SELECT * FROM tabqy_user a WHERE a.user_id NOT IN (SELECT userlocation_user_id FROM tabqy_userlocations b where b.userlocation_company_id='$company_id') and a.user_company_id='$company_id' and a.user_role=3 and a.user_status='1' ";
        $query = $this->db->query($sql); 
		$data['users']=$this->db->result($query);
		$data['action']="";
		$data['title']="User location list";
		...
		view_loader('admin/userlocation/company.html',$data);
	}
	...
	Public function add(){
		...
		if($this->validation->validation_check()== FALSE){
            ...
            $data['send_data']['userlocation_company_id']=$_POST['userlocation_company_id'];
