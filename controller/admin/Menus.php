...
public function __construct()
{
	...
	//	unset($_SESSION['portion_array']);
		...
		$data['brand_id']=$this->url->segment(4);
		$restaurant_id=$this->url->segment(4);
		// List of categories
		...
		$country_id=$_POST['item_country_id'];
		$this->db->select('tabqy_countries.country_language_assign');
		$this->db->from('tabqy_countries');
		$this->db->where("tabqy_countries.country_code",$country_id);
		$query = $this->db->get();
		$countries=$this->db->row($query);
		$country_language_assign=explode(',',$countries['country_language_assign']);	
		$item=array();
		$item=$_POST;
		...
		$item_table['item_prepration_cost']=$item['item_prepration_cost'];
		$item_table['item_brand_id']=$restaurant_id;
        $item_table['item_country_id']=$country_id;
		$item_table['item_company_id']=$company_id;
	   ...
	   $allowedExts = array("gif", "jpeg", "jpg", "png");
		...
		if(isset($item['item_status']) && $item['item_status'] == '1'){
		...
		foreach ($country_language_assign as $language) {
                $edit              = ($language == $_SESSION['lang_code']) ? '1' : '0';
                $add_data_language = array(
                    "item_language_item_id" => $item_id,
                    "item_language_language_code" => $language,
                    "item_language_item_name"=>$_POST['item_name'],
                    "item_language_item_description"=>$_POST['item_description'],
                    "item_language_edit" => $edit,
                    "item_language_status" => '1'
                );
                
               $this->db->insert('tabqy_item_language', $add_data_language);
           }
		...
		redirect('admin/menus/resturant/'.$restaurant_id.'/'.$country_id.'/'.$company_id);
	   ...
	   $allowedExts = array("gif", "jpeg", "jpg", "png");
		...
		unset($_SESSION['combo_array']);
		set_flesh('success_massage', 'New Combo item added successfully');
	...
	redirect('admin/menus/resturant/'.$restaurant_id.'/'.$country_id.'/'.$company_id);
		...
		view_loader('admin/items/add-item-combo.html',$data);
	}
...
//	print_r($_SESSION['portion_array']);die;
	...
	Public function sessionreset(){
		...
		//$restaurant_id=$_SESSION['admin_userdata']['restaurant_id'];
	 	$item_per_page      = 10; //item to display per page
		$page_url           = baseurl."admin/menus/index/"; //URL
		...
		$this->db->flush_cache();
		$this->db->select('tabqy_item.*,tabqy_categories.category_name,tabqy_resturantbrand.resturantbrand_name');
		$this->db->from('tabqy_item');
		if($search!='0'){
			$this->db->where("tabqy_item.item_name LIKE '%$search%'");
		}
		...
		view_loader('admin/items/list.html',$data);
	}
		...
		$data['company_id']=$company_id;
	 	$item_per_page      = 10; //item to display per page
		$page_url           = baseurl."admin/menus/resturant/".$restaurant_id.'/'.$country_id.'/'.$company_id; //URL
		...
		$page_position = (($page_number-1) * $item_per_page); //get starting position to fetch the records
		$this->db->flush_cache();
		$this->db->select('tabqy_item.*,tabqy_categories.category_name,tabqy_resturantbrand.resturantbrand_name,tabqy_countries.country_language_assign');
		$this->db->from('tabqy_item');
		if($search!='0'){
			$this->db->where("tabqy_item.item_name LIKE '%$search%'");
		}
		$this->db->join('tabqy_categories','tabqy_item.item_category_id=tabqy_categories.category_id','inner');
		$this->db->join('tabqy_resturantbrand', "tabqy_resturantbrand.resturantbrand_id=tabqy_categories.category_brand_id", "inner");	
		$this->db->join('tabqy_countries','tabqy_countries.country_code=tabqy_item.item_country_id');
		$this->db->where("tabqy_item.item_brand_id",$restaurant_id);
		$this->db->where("tabqy_item.item_company_id",$company_id);
		$this->db->where("tabqy_item.item_country_id",$country_id);
		...
		$data['start']=$start;
		$data['end']=($start+count($data['results'])-1);
		view_loader('admin/items/list_item.html',$data);
	}
	...
	Public function edit(){
		...
		$editdata = $this->item_edit_data($item_id,$language_code);
		$restaurant_id=$editdata['item_data']['item_brand_id'];
		$country_id=$editdata['item_data']['item_country_id'];
		$company_id=$editdata['item_data']['item_company_id'];
		  if($_SESSION['userdata']['user_role'] != 0 ){ 
			if($this->privillage['edit'] != 1 ){
			  set_flesh('success_massage', "You don't have permission to perform this action");
			  redirect('admin/menus/resturant/'.$restaurant_id.'/'.$country_id.'/'.$company_id);		
			exit();
			}
			} 
			...
			$item_table['item_defaultprice']=$item['item_default_price'];
			 $item_table['item_type']='0';
			if(isset($item['item_admin_price']) && !empty($item['item_admin_price'])){
			   $item_table['item_branchprice']='1';
			}
			if(isset($item['item_hide_pos'])&& isset($item['item_hide_web']))
			{
			...
			if($item['item_status'] == '1'){
             ...
             $description = $item_table['item_description'];
			 $name = $item_table['item_name'];
              unset($_POST['submit']);
            ...
            if ($_SESSION['lang_code'] == $language_code) {
               ...
               $this->db->flush_cache();
			    ...
			    "tabqy_item_language.item_language_item_description" => $description,
                "tabqy_item_language.item_language_item_name" => $name,
                "tabqy_item_language.item_language_edit" => '1'
               );
                $this->db->where('item_language_item_id', $item_id);
	            $this->db->where('item_language_language_code', $language_code);
	            $this->db->update('tabqy_item_language', $edit_data_language);
			...
			// save data in tabqy_itempriceofbranch table
			if(isset($item['item_admin_price']) && !empty($item['item_admin_price'])){
				$item_admin_prices=array();
				foreach($item['item_admin_price'] as $key=>$item_admin_price){
							$item_admin_prices['itempriceofbranch_branch_id']= $key	;
							$item_admin_prices['itempriceofbranch_item_id']= $item_id;
							$item_admin_prices['itempriceofbranch_price']=$item_admin_price;
							$item_admin_prices['itempriceofbranch_status']='1';
				$this->db->insert('tabqy_itempriceofbranch', $item_admin_prices);
				}
			}
			// save data in tabqy_discount table
			...
			// save data in tabqy_item_associated table
			if(isset($_SESSION['associative_array']) && !empty($_SESSION['associative_array'])){
			    		$this->db->where('associated_item_id', $item_id);
		              $this->db->delete('tabqy_item_associated'); 
			...
			// save data in tabqy_itemportion table
			if(isset($_SESSION['portion_array']) && !empty($_SESSION['portion_array'])){
			    		$this->db->where('itemportion_item_id', $item_id);
	         	$this->db->delete('tabqy_itemportion'); 
				$portionsarray=array();
		...
		if(!empty($editdata['associated_data'] )){
			...
			foreach($editdata['associated_data'] as $k=>$associated){
		...
		$data['item_prepration_cost'] = isset($_POST['item_prepration_cost']) ? $_POST['item_prepration_cost'] : $editdata['item_data']['item_prepration_cost'];
	...
	Public function edit_data(){
		...
		$company_id=$editdata['item_data']['item_company_id'];
		  if($_SESSION['userdata']['user_role'] != 0 ){ 
			...
			$this->db->from("tabqy_item");
			$this->db->join('tabqy_categories','tabqy_item.item_category_id=tabqy_categories.category_id','inner');
			$this->db->join('tabqy_item_language','tabqy_item_language.item_language_item_id=tabqy_item.item_id');
	        $this->db->where('tabqy_item_language.item_language_language_code', $language_code);
	        
			$rs_item = $this->db->get();
			$data['item_data'] = $this->db->result($rs_item);
          // echo "<pre>";
            //print_r($data['item_data'][0]['item_language_item_name']);die; 
			$this->db->where("itempriceofbranch_item_id",$id);
	...
	$this->db->join('tabqy_quantity','tabqy_quantity.  quantity_id=tabqy_comboitem.comboitem_portion_id','LEFT');
		
			$rs_combo = $this->db->get();
		//	print_r($rs_combo);die;
...
if(isset($combo_items)){
			...
			foreach($combo_items as $k=>$combo_item){
			    if($combo_item['comboitem_portion_id']>0){
		...
		//	$item_table['item_brand_id']=$restaurant_id;
			...
			view_loader('admin/items/edit-combo.html',$data);
	...
	date 	 : 23/03/2018
	===============*/
	public function item_edit_data($id,$language_code=""){
		$data  = array();
		$this->db->select("*");
		$this->db->from("tabqy_item");
        $this->db->join('tabqy_item_language','tabqy_item_language.item_language_item_id=tabqy_item.item_id');
	...
	Public function view($id){
		   ...
		   $data['action']="";
			...
			$language_code = $_SESSION['lang_code'];
			$this->db->select("*");
			$this->db->from("tabqy_item");
           $this->db->join('tabqy_item_language','tabqy_item_language.item_language_item_id=tabqy_item.item_id');
            $this->db->where('tabqy_item_language.item_language_language_code', $language_code);
			...
			$rs_discount_price = $this->db->get();
			$data['discount_data'] = $this->db->result($rs_discount_price);
			$data['restaurant_id']=$data['item_data'][0]['item_brand_id'];
			...
			view_loader('admin/items/view-item.html',$data);
		}
