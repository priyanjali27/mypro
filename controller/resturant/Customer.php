...
class Customer extends Controller{
	...
	Public function index(){
		...
		$this->db->select($tbl.'.*');
		$this->db->from($tbl);
		$this->db->where('customer_resturant_id',$restaurant_id);
		$this->db->order_by($tbl.'.customer_id');
		...
		view_loader('resturant/customer/index.html',$data);
	}
	...
	Public function add(){
		...
		if($this->validation->validation_check()== FALSE){
		    ...
		    if(!empty($data['error_msg'])){
				...
				$insert_id=$this->db->insert('tabqy_'.$pre, $data['send_data']);
				$restaurant_id=$_SESSION['resturant_userdata']['restaurant_id'];
				$data['new_data']['customer_resturant_id']=$restaurant_id;
				$data['new_data']['customer_customer_id']=$insert_id;
				$data['new_data']['customer_resturant_relation_status']=1;
				$data['new_data']['customer_resturant_relation_created'] = date('Y-m-d H:i:s');
				$this->db->insert('tabqy_customer_resturant_relation', $data['new_data']);
